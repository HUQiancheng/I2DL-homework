...
class RescaleTransform:
    ...
    def __call__(self, image):
        ...
        assert type(image) == np.ndarray, "The input image needs to be a numpy array! Make sure you dont send the string path."
        ret_image = image.copy()  # Create a copy of the image to avoid changing it in-place
        ret_image = (ret_image - self._data_min) / (self._data_max - self._data_min)  # Scale from [self._data_min, self._data_max] to [0, 1]
        ret_image = ret_image * (self.max - self.min) + self.min  # Scale from [0, 1] to [self.min, self.max]
        return ret_image
