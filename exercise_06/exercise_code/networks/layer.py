...
class LeakyRelu:
    ...
    def forward(self, x):
        ...
        ########################################################################
        # Compute the output using the Leaky ReLU function
        out = np.where(x > 0, x, self.slope * x)
        cache = x  # Store input x in cache for use in the backward pass
        return out, cache
    ...
    def backward(self, dout, cache):
        ...
        ########################################################################
        x = cache  # Retrieve input x from cache
        dx = dout * np.where(x > 0, 1, self.slope)  # Compute gradient
...
class Tanh:
    ...
    def forward(self, x):
        ...
        ########################################################################
        out = np.tanh(x)
        cache = out  # Store output in cache for use in the backward pass
        return out, cache
    ...
    def backward(self, dout, cache):
        ...
        ########################################################################
        tanh_out = cache  # Retrieve output of tanh from cache
        dx = dout * (1 - tanh_out ** 2)  # Compute gradient
        return dx
        ########################################################################
        #                           END OF YOUR CODE                           #
        ########################################################################
