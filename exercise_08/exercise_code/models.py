...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),  # Add Dropout for regularization
            nn.Linear(self.hparams["n_hidden"] // 2, self.latent_dim)
        )
...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams, latent_dim=20, output_size=28 * 28):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),  # Add Dropout for regularization
            nn.Linear(self.hparams["n_hidden"] // 2, self.hparams["n_hidden"]),
            nn.ReLU(),
            nn.Dropout(0.3),  # Add Dropout for regularization
...
class Classifier(nn.Module):
    ...
    def __init__(self, hparams, encoder):
        ...
        self.model = nn.Sequential(
            nn.Linear(self.encoder.latent_dim, self.hparams["n_hidden"] // 2),
            nn.ReLU(),
            nn.Dropout(0.3),  # Add Dropout for regularization
            nn.Linear(self.hparams["n_hidden"] // 2, self.hparams["n_hidden"] // 4),
            nn.ReLU(),
            nn.Dropout(0.3),  # Add Dropout for regularization
            nn.Linear(self.hparams["n_hidden"] // 4, 10)  # Assuming 10 classes for MNIST
        )
