...
class EncoderBlockValueTest(UnitTest):
    def __init__(self):
        ...
        task_path = join(tensor_path, 'task_6')
        ...
        if encoder_block.multi_head.dropout is None:
            encoder_block.load_state_dict(torch.load(join(task_path, 'model_a.pt')))
        else:
            encoder_block.load_state_dict(torch.load(join(task_path, 'model_b.pt')))
